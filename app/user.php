...
class update_user_info {
    ...
    public function StoreUserInfo($name, $email, $password) {
        ...
        $stmt = $this->conn->prepare("INSERT INTO user(username, email, password, salt) VALUES(?, ?, ?, ?)");
        ...
        if ($result) {
            $stmt = $this->conn->prepare("SELECT name, email, password, salt FROM user WHERE email = ?");
    ...
    public function VerifyUserAuthentication($email, $password) {
        ...
        $stmt = $this->conn->prepare("SELECT username, email, password, salt FROM user WHERE email = ?");
